...
void LinearIndicator::drawLabel(QPainter *p, QString label, qreal x, qint32 height, qint32 halfTextHeight)
{
    ...
    if (m_orientation == HorizontalTop) {
        p->drawText(QPointF(x - fontMetrics.lineWidth() / 2, height), label);
    } else if (m_orientation == HorizontalBottom) {
        p->scale(1, -1);
        p->drawText(QPointF(x - fontMetrics.lineWidth() / 2, -height + halfTextHeight), label);
        p->scale(1, -1);
    } else if (m_orientation == VerticalRight) {
        p->rotate(-90);
        p->scale(1, -1);
        ...
        p->rotate(90);
        p->drawText(QPointF(boundingRect().width() - fontMetrics.lineWidth(), -x + halfTextHeight), label);
        p->rotate(-90);
    }
}
